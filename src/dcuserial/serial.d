module dcuserial.serial;

import std.system : Endian, endian;
import std.traits : isArray, isDynamicArray, isStaticArray, isAssociativeArray, ForeachType, KeyType, ValueType,
    isIntegral, isFloatingPoint, isSomeChar, isType, isCallable, isPointer, hasUDA, getUDAs;
import std.typecons : isTuple;
import std.algorithm.searching : canFind;

import dcubuffer.buffer : canSwapEndianness, Buffer, BufferOverflowException;
import dcubuffer.memory : xalloc, xfree;
import dcubuffer.varint : isVar;
    ...
    mixin({
        ...
        foreach (member; __traits(allMembers, T))
        {
            static if (is(typeof(mixin("T." ~ member))))
            {
                ...
                static if (isType!M && !isCallable!M && (hasUDA!(__traits(getMember,
                    T, member), Include) || ([__traits(derivedMembers,
                        T)].canFind(member) && !__traits(compiles, {
                        mixin("auto test=T." ~ member ~ ";");
                    }) && // static members
                    !__traits(compiles, {
                        mixin("auto test=T.init." ~ member ~ "();");
                    }) && // properties
...
{
    static if (isVar!L)
        serializeImpl!(cast(EndianType) endianness, L.Type, EndianType.var,
                L.Type, EndianType.var, T)(buffer, value);
    else
        serializeImpl!(cast(EndianType) endianness, L,
                cast(EndianType) lengthEndianness, L, cast(EndianType) lengthEndianness, T)(buffer,
                value);
}

void serializeImpl(EndianType endianness, OL, EndianType ole, CL, EndianType cle, T)(
        Buffer buffer, T value)
{
    ...
    {
        static if (__traits(hasMember, T, "serialize") && __traits(compiles,
...
void serializeLength(EndianType endianness, L)(Buffer buffer, size_t length)
{
    static if (L.sizeof < size_t.sizeof)
        serializeNumber!(endianness, L)(buffer, cast(L) length);
    else
        serializeNumber!(endianness, L)(buffer, length);
}

void serializeArray(EndianType endianness, OL, EndianType ole, T)(Buffer buffer, T array)
        if (isArray!T)
{
    static if (canSwapEndianness!(ForeachType!T) && !is(ForeachType!T == struct)
            && !is(ForeachType!T == class) && endianness != EndianType.var)
    {
...
void serializeAssociativeArray(EndianType endianness, OL, EndianType ole, T)(Buffer buffer, T array)
...
void serializeTuple(EndianType endianness, OL, EndianType ole, T)(Buffer buffer, T tuple)
        if (isTuple!T)
{
        ...
        static if (!is(typeof(__custom)))
            mixin({
                ...
                static if (hasUDA!(__traits(getMember, T, member), LengthImpl))
                    ...
                    immutable e = "L, le, " ~ length.type ~ ", " ~ (length.endianness == -1
                        ? "endianness" : "EndianType." ~ (cast(EndianType) length.endianness)
                        .to!string);
                }
                else
                    {
                ...
                static if (hasUDA!(__traits(getMember, T, member), NoLength))
                    immutable ret = "dcuserial.serial.serializeArray!(endianness, L, le, M)(__buffer, __container."
                        ~ member ~ ");";
                else static if (hasUDA!(__traits(getMember, T, member), Var))
                    immutable ret = "dcuserial.serial.serializeImpl!(EndianType.var, "
                        ~ e ~ ", M)(__buffer, __container." ~ member ~ ");";
                else static if (hasUDA!(__traits(getMember, T, member), BigEndian))
                    immutable ret = "dcuserial.serial.serializeImpl!(EndianType.bigEndian, "
                        ~ e ~ ", M)(__buffer, __container." ~ member ~ ");";
                else static if (hasUDA!(__traits(getMember, T, member), LittleEndian))
                    immutable ret = "dcuserial.serial.serializeImpl!(EndianType.littleEndian, "
                        ~ e ~ ", M)(__buffer, __container." ~ member ~ ");";
                else
                    immutable ret = "dcuserial.serial.serializeImpl!(endianness, "
                        ~ e ~ ", M)(__buffer, __container." ~ member ~ ");";

                static if (!hasUDA!(__traits(getMember, T, member), Condition))
                    return ret;
                else
                    return "with(__container){if(" ~ getUDAs!(__traits(getMember,
                        T, member), Condition)[0].condition ~ "){" ~ ret ~ "}}";
...
{
    static if (isVar!L)
        return deserializeImpl!(cast(EndianType) endianness, L.Type,
                EndianType.var, L.Type, EndianType.var, T)(buffer);
    else
        return deserializeImpl!(cast(EndianType) endianness, L,
                cast(EndianType) lengthEndianness, L, cast(EndianType) lengthEndianness, T)(buffer);
}
...
T deserializeImpl(EndianType endianness, OL, EndianType ole, CL, EndianType cle, T)(Buffer buffer)
{
    ...
    {
        return deserializeDynamicArray!(endianness, OL, ole, T)(buffer,
                deserializeLength!(cle, CL)(buffer));
    }
    else static if (isAssociativeArray!T)
    {
        return deserializeAssociativeArray!(endianness, OL, ole, T)(buffer,
                deserializeLength!(cle, CL)(buffer));
    }
    else static if (isTuple!T)
    {
    ...
    else static if (is(T == class) || is(T == struct))
    {
        ...
        static if (is(T == class))
            ret = new T();
        static if (__traits(hasMember, T, "deserialize") && __traits(compiles,
...
{
    static if (L.sizeof > size_t.sizeof)
        return cast(size_t) deserializeNumber!(endianness, L)(buffer);
    else
        return deserializeNumber!(endianness, L)(buffer);
}

T deserializeStaticArray(EndianType endianness, OL, EndianType ole, T)(Buffer buffer)
        if (isStaticArray!T)
{
    T ret;
...
T deserializeDynamicArray(EndianType endianness, OL, EndianType ole, T)(Buffer buffer, size_t length)
        if (isDynamicArray!T)
{
    T ret;
    foreach (i; 0 .. length)
...
T deserializeAssociativeArray(EndianType endianness, OL, EndianType ole, T)(
        Buffer buffer, size_t length) if (isAssociativeArray!T)
{
    T ret;
    foreach (i; 0 .. length)
    {
        ret[deserializeImpl!(endianness, OL, ole, OL, ole, KeyType!T)(buffer)] = deserializeImpl!(endianness,
                OL, ole, OL, ole, ValueType!T)(buffer);
    }
    return ret;
}

T deserializeNoLengthArray(EndianType endianness, OL, EndianType ole, T)(Buffer buffer)
        if (isDynamicArray!T)
{
    T ret;
    try
...
T deserializeTuple(EndianType endianness, OL, EndianType ole, T)(Buffer buffer)
        if (isTuple!T)
{
    T ret;
    foreach (i, U; T.Types)
    ...
    foreach (member; Members!(T, EncodeOnly))
    {
        ...
        static if (!is(typeof(__custom)))
            mixin({
                ...
                static if (hasUDA!(__traits(getMember, T, member), NoLength))
                    immutable ret = "__container." ~ member
                        ~ "=dcuserial.serial.deserializeNoLengthArray!(endianness, L, le, M)(__buffer);";
                else static if (hasUDA!(__traits(getMember, T, member), Var))
                    immutable ret = "__container." ~ member
                        ~ "=dcuserial.serial.deserializeImpl!(EndianType.var, "
                        ~ e ~ ", M)(__buffer);";
                else static if (hasUDA!(__traits(getMember, T, member), BigEndian))
                    immutable ret = "__container." ~ member
                        ~ "=dcuserial.serial.deserializeImpl!(EndianType.bigEndian, "
                        ~ e ~ ", M)(__buffer);";
                else static if (hasUDA!(__traits(getMember, T, member), LittleEndian))
                    immutable ret = "__container." ~ member
                        ~ "=dcuserial.serial.deserializeImpl!(EndianType.littleEndian, "
                        ~ e ~ ", M)(__buffer);";
                else
                    immutable ret = "__container." ~ member
                        ~ "=dcuserial.serial.deserializeImpl!(endianness, " ~ e ~ ", M)(__buffer);";
...
@("numbers") unittest
{
    ...
    version (LittleEndian)
        assert(12.serialize() == [12, 0, 0, 0]);
    version (BigEndian)
        assert(12.serialize() == [0, 0, 0, 12]);
...
@("arrays") unittest
{
    ...
    assert([1, 2, 3].serialize!(Endian.bigEndian, uint)() == [
...
@("associative arrays") unittest
{
    ...
    int[ushort] test;
    test[1] = 112;
    assert(test.serialize!(Endian.bigEndian, uint)() == [
        0, 0, 0, 1, 0, 1, 0, 0, 0, 112
    ]);

    test = deserialize!(int[ushort], Endian.bigEndian, ubyte)([
        1, 0, 0, 0, 0, 0, 55
    ]);
...
@("tuples") unittest
{
    ...
    assert(tuple(1, "test").serialize!(Endian.bigEndian, ushort)() == [
        0, 0, 0, 1, 0, 4, 't', 'e', 's', 't'
    ]);
...
@("attributes") unittest
{
    ...
    assert(test1.serialize() == [0, 0, 0, 1, 2, 0, 6]);
    assert(deserialize!Test1([0, 0, 0, 1, 4, 12]) == Test1(1, 0, 2, 6));
    ...
    assert(test2.serialize!(Endian.bigEndian, uint)() == [
    ...
    assert(test3.serialize!(Endian.bigEndian)() == [
        2, 0, 1, 0, 2, 0, 0, 3, 0, 4
    ]);
